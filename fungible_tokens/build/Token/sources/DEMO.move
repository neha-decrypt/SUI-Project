module Token::DEMO {
    ...
    fun init(witness: DEMO, ctx: &mut TxContext) {
        let (treasury, metadata) = coin::create_currency(witness, 9, b"DE", b"DEMO", b"", option::none(), ctx);
        transfer::public_freeze_object(metadata);
        transfer::public_transfer(treasury, tx_context::sender(ctx));
    }

    public entry fun mint(
        treasury: &mut coin::TreasuryCap<DEMO>, amount: u64, recipient: address, ctx: &mut TxContext
    ) {
        coin::mint_and_transfer(treasury, amount, recipient, ctx);
    }

    public entry fun transfer(mut coins: vector<coin::Coin<DEMO>>, amount: u64, recipient: address, ctx: &mut TxContext) {
